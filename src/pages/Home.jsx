...
import { setShowPayment } from "../store/paymentSlice";
...
const Home = () => {
  ...
  const progressSteps = [
    ...
    { step: "STEP 2", title: "Payment details", color: "#9CD7BB" },
    {
      step: "STEP 3",
      title: "Confirmation",
      color: showCharger ? "#C1E296" : "#7F7F7F",
    },
  ];
  ...
  return (
    <div className="min-h-screen font-satoshi bg-[#161B21] text-white p-4">
      {/* Logo */}
      <div className="flex justify-center">
        <img src={logo} alt="BLIP Logo" className="w-32 md:w-40" />
      </div>

      {/* Title */}
      <h1 className="text-center text-xl md:text-3xl font-medium mb-12">
      ...
      {showCharger ? (
        ...
        <div className={`${showLocationPopup ? "hidden" : "block" } max-w-4xl mx-auto bg-[#1D232C] rounded-2xl p-10 md:p-14`}>
          <div className="flex flex-col items-center text-center border-gray-600">
            ...
            {/* Desktop Button */}
            <button
              className="hidden md:flex bg-[#3B4758] py-3.5 px-6 rounded-[10px] text-lg items-center gap-1.5"
      ...
      {showLocationPopup && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 animate-fadeIn">
          <div className="bg-[#1D232C]  py-6 rounded-[20px] text-white max-w-4xl text-center">
            {/* Popup Header */}
            <div className="px-16 ">
